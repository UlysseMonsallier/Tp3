...
# Remplacement de la disposition du panneau de gauche pour utiliser grid
...
# Champs angle et vitesse avec grid pour un meilleur alignement
...
# Les labels de résultat regroupés dans un petit frame
result_frame = tk.Frame(cadre_textes)
result_frame.grid(row=4, column=0, sticky="we", padx=5, pady=(0,8))
...
# Boutons regroupés et alignés verticalement dans un frame
btn_frame = tk.Frame(cadre_textes)
btn_frame.grid(row=5, column=0, sticky="we", padx=5, pady=(0,8))
...
tk.Label(cadre_textes, text="Entrez un multiplicateur d'étape").grid(row=7, column=0, sticky="w", padx=5, pady=(2,2))
champ_multi = tk.Entry(cadre_textes, width=30)
champ_multi.grid(row=8, column=0, sticky="we", padx=5, pady=(0,8))
